...
let restaurantFinderApplication = {
  ...
  findAvailableRestaurants: function (numberOfPeople) {
    return (restaurants.filter((i)=>(i.totalSeats - i.numberOfCustomers) >= numberOfPeople)).map((i)=>i.name);
  },
  findRestaurantServingDish: function (dishName) {
    return (restaurants.filter((i)=>i.menu.some((j)=>j == dishName))).map((i)=>i.name);
  },
  countNumberOfRestaurantsInArea: function (area) {
    return (restaurants.filter((i)=>i.address.area == area)).length;
  },
};
